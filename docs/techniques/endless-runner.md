...
- **Missions on screen** (`missionView()` in `createRunnerGame.js`, `setMissions` / `missionComplete` / `renderOrders` in `createRunnerHud.js`):
  - The countdown shows a MISSION BRIEFING, and during a run an ORDERS card under the score ticket tracks the three orders live. On touch it shows one line: the order closest to done.
  - Rows flash on progress, pulse pink at ≥75%, and strike through when done. A completed order fires a centre "MISSION COMPLETE" stamp (queued if several land together).
  - The start ticket lists the orders with rewards, and the game-over ticket shows this run's progress. It snapshots `trackRun` before `endRun` swaps finished orders for new ones.
- **Game-over shake:** `die()` plays one real-time death jolt, then `controls.setShakeEnabled(false)` blocks explosions and drones from shaking the results screen. `showGameOver` clears any shake still running, and `resetRun` re-enables it.
- **Soundtrack** (`audio/createMusicPlayer.js`, `audio/musicCatalog.js`, `ui/runner/musicPlayerUi.js`): an EA FC-style licensed-music catalog for the tracks in `public/game music`. Covers come from the ID3 art, extracted to `covers/*-512.jpg` / `*-128.jpg`. Two `<audio>` decks stream through Web Audio (per-deck gains for crossfade, master volume, a pause duck, an analyser for the visualizer); gains live in Web Audio because iOS ignores `element.volume`. The game sets a context (`menu` / `run` / `paused` / `dead`), and settings decide whether music plays in menus or during runs. The *Synth score* source hands runs back to the procedural `createAdaptiveMusic`. The start ticket has a MUSIC screen: now playing with a spinning vinyl, visualizer, seek bar and transport; the tracklist with in-rotation toggles; and source, volume, menus / runs, pop-ups and crossfade settings, persisted in localStorage. The HUD has a tiny mini player (album art, title, eq bars, progress), shown when the soundtrack source is active. On desktop it sits in the header row just left of ABOUT (`placeMiniMusic` measures `.app-header-actions`); on touch it sits beside PAUSE. The ambience loops (`createAudioButton`) play through a Web Audio gain that follows the SFX volume, because iOS ignores `element.volume`. The pause screen has a mini player, a now-playing card slides in on every track change, N skips, and the OS media keys work via Media Session.
...
- **Sky Run** (flying-car power-up; `createFlightMode.js`, `models/createFlyingCarModel.js`). The car is the project's Quadra (`world.car`), cloned with transforms unfrozen, yawed so its nose points +X, centred, and fitted with hover pads, twin rear jets and side cannon pods (the muzzles). A procedural car is the fallback when the car feature is off. Flight steering is a spring-damper (`RUNNER.flightSpring`): a lane switch reaches 90% in ~0.4 s with ~0.2 m overshoot. The car banks from lateral velocity plus acceleration (leans in, counter-leans on settle), noses into its travel and pitches on climbs, all smoothed. The chase cam trails with its own softer follow and light roll, plus a small FOV punch on fast strafes. Steering into the wall at an outer lane does a barrel roll. a rare `flycar` pickup appears in reward lines from 380 m, then every 900–1400 m (seeded). Collecting it switches to a Star Fox-style rail shooter:
  - `controls.setFlight(true)` gives the same inputs new meanings: A/D strafe across wider sky lanes (`RUNNER.flightLaneZ`), jump / slide climb or dive between altitude tiers (`flightAltitudes`). Lanes (z 20 / 26.5 / 33) and tiers (16 / 20.5 / 25 m above the floor) were chosen from a raycast scan of the city and billboard models. Cables and signs cross the street at 7–13 m, a left-side sign reaches z 17 at the top tier, and the right facades start at z 36, so the whole car stays inside the clear z 17.5–35.5 corridor. There is no gravity, and a chase camera blends in from first person over ~1.1 s while the car swoops in ahead.
  - Hitscan still fires from the camera through the crosshair; `weapon.setMuzzleProvider` sends tracers from the car's alternating cannons. `weapon.setVehicleMode` gives the cannons infinite ammo (no hidden reloads), 15% faster fire and 3.5× thicker tracers. It also starts the ray level with the car (`rayOffset`) so geometry between the chase cam and the car can't eat shots. Specials launch from the nose.
  - Drones fight at altitude via a virtual `player.floorY` 4.5 m below the car.
  - Ground obstacles are cleared and their spawner paused. The sky is obstacle-free: nothing blocks the car, and only drones damage it. Shard lines and climbing arcs are threaded through the lanes.
  ...
  - At 0 hull the car explodes and its wreck tumbles away. A crash cinematic follows (`controls.startCrashCam`, `CRASH_CAM_TIME` 1.7 s real time): the camera eases from the chase view to a three-quarter close-up of the wreck and narrows the FOV to 45°. Meanwhile the world runs at 0.3× (`CRASH_TIME_SCALE`), input is locked and the obstacle spawner is paused. Then the runner drops back to the street (`setFlight(false)`: gravity plus the first-person blend) with 2.6 s of invulnerability, a slow-mo beat and a 70 m clear street.
  - The car is in `collisionHideObjects`, shift with the floating origin, and are warmed at startup. Dev shortcut: `__app.runner.startFlight()`.
