...
- **Moebius comic mode** (`src/tsl/moebius.js`): an alternate post style that replaces the neon grade and film grain in `rebuildSteadyOutput` (`pipeline.setVisualStyle("moebius" | "neon")`). It draws ink outlines from log-depth edges plus a luminance Sobel on the scene colour, with the Sobel off on mobile via `performanceProfile.moebiusColorEdges`. Luminance is banded in perceptual space and pulled toward a pastel ramp. The darkest bands get cross-hatching. It adds distance haze, a flat teal-to-sand sky, and warm paper grain. Bloom is added after the ink so neon stays vivid. The player toggles it with STYLE on the start ticket or in Settings → Look, and the choice is saved in localStorage.
